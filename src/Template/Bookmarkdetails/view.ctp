<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Bookmarkdetail $bookmarkdetail
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Bookmarkdetail'), ['action' => 'edit', $bookmarkdetail->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Bookmarkdetail'), ['action' => 'delete', $bookmarkdetail->id], ['confirm' => __('Are you sure you want to delete # {0}?', $bookmarkdetail->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Bookmarkdetails'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Bookmarkdetail'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Bookmarks'), ['controller' => 'Bookmarks', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Bookmark'), ['controller' => 'Bookmarks', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="bookmarkdetails view large-9 medium-8 columns content">
    <h3><?= h($bookmarkdetail->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Bookmark') ?></th>
            <td><?= $bookmarkdetail->has('bookmark') ? $this->Html->link($bookmarkdetail->bookmark->title, ['controller' => 'Bookmarks', 'action' => 'view', $bookmarkdetail->bookmark->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($bookmarkdetail->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Created') ?></th>
            <td><?= h($bookmarkdetail->created) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Modified') ?></th>
            <td><?= h($bookmarkdetail->modified) ?></td>
        </tr>
    </table>
    <div class="row">
        <h4><?= __('Content') ?></h4>
        <?= $this->Text->autoParagraph(h($bookmarkdetail->content)); ?>
    </div>
</div>
